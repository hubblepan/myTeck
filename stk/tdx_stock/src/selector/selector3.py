...
def main(position=-1, plate='sh'):
    ...
    for stock_code in code_list:
        try:
            ...
            s_close_5 = kline['close'][position - 4: position].astype('float')
            ...
            if vol[position] / max(sma_vol_10[position], sma_vol_20[position]) > 1.5:
                continue

            # 没有穿过均线的不要
            if s_low > max(sma_close_5[position], sma_close_10[position], sma_close_20[position]):
                continue
